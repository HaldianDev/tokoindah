const SHIPPING_COST_PER_KG = P__SHIPPING_COST_PER_KG;
let cart = [];
    ...
    if (existing) {
        if (existing.quantity + 1 > stock) {
            Swal.fire({
                ...
                text: `Maaf, stok maksimal produk ini adalah ${stock} unit.`,
            });
            return;
        }
        existing.quantity += 1;
...
function togglePaymentFields() {
    ...
    const paymentProofInput = document.getElementById('paymentProofFile');
    ...
    if (method === 'credit') {
        ...
        paymentProofWrapper.classList.add('hidden');
        ...
        paymentProofWrapper.classList.remove('hidden');
...
function handleCheckoutSubmit(e) {
    ...
    if (paymentProofInput && paymentProofInput.files.length > 0) {
        formData.append('payment_proof_file', paymentProofInput.files[0]);
    }

    fetch(P__CHECKOUT_URL, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            Swal.fire({
                ...
                text: data.message || 'Terima kasih telah berbelanja bersama kami.',
                showConfirmButton: false,
                timer: 3000
            }).then(() => {
                window.location.href = P__CATALOG_URL;
            });
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Gagal membuat pesanan!',
                text: data.message || 'Periksa kembali data masukan Anda.',
            });
            btn.disabled = false;
