...
@section('sidebar_nav')
    ...
    <button class="sidebar-item" data-section="sec-admin-account-settings" data-title="Pengaturan Akun" data-breadcrumb="Pengaturan"
        onclick="switchSection('sec-admin-account-settings','Pengaturan Akun','Pengaturan')">
        <div class="sidebar-item-icon" style="background:rgba(2,132,199,0.12);color:#38BDF8;"><i class="fa-solid fa-user-cog"></i></div>
        Pengaturan Akun
    </button>
...
<div class="dash-section" id="sec-overview">
    <div class="section-header">
        <div>
            ...
            <p class="section-subtitle">Ringkasan real-time sistem toko {{ $appSettings->site_name }}</p>
        </div>
                    ...
                    <tr>
                        ...
                        <td style="text-align:center;">
                            <div style="display:inline-flex;gap:6px;align-items:center;">
                                <button onclick="openEditProductModal({{ $p->id }})" class="btn-action btn-outline btn-sm btn-icon" title="Edit">
                        ...
                        <td style="text-align:center;">
                            @if($ord->status === 'verifying_payment')
                                <div style="display:flex; gap:8px; justify-content:center; align-items:center;">
                                    <a href="{{ route('file.serve', ['path' => base64_encode($ord->payment_proof_path)]) }}" target="_blank" class="btn-action btn-outline btn-sm">
...
<div class="dash-section" id="sec-cashier">
    <div class="section-header">
        <div>
            ...
            <p class="section-subtitle">Transaksi penjualan langsung di toko offline {{ $appSettings->site_name }}.</p>
        </div>
    </div>
                ...
                <div class="cashier-product-card" data-name="{{ strtolower($prod->name) }}" data-category="{{ $prod->category->name ?? '' }}"
                    ...
                    <div style="display:flex;flex-direction:column;flex:1;">
                        ...
                        <div style="font-size:0.75rem;color:#64748B;margin-top:2px;">
                            Stok: 
                            @if($prod->stock <= 0)
                                <strong style="color:#EF4444;">Habis</strong>
                            @else
                                <strong style="color:#059669;">{{ $prod->stock }} unit</strong>
                            @endif
                        </div>
                    </div>
                    @if($prod->stock <= 0)
                        <button disabled class="btn-action btn-secondary btn-sm" style="width:100%;justify-content:center;margin-top:6px;padding:6px 0;">
...
{{-- ============================================ --}}
{{-- SECTION: PENGATURAN AKUN (Admin)             --}}
{{-- ============================================ --}}
<div class="dash-section" id="sec-admin-account-settings">
    <div class="section-header">
        <div>
            <h1 class="section-title"><i class="fa-solid fa-user-cog" style="color:#38BDF8;"></i> Pengaturan Akun</h1>
            <p class="section-subtitle">Kelola informasi profil, kata sandi, dan foto profil Anda.</p>
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="table-card">
            <div class="table-card-header">
                <div class="table-card-title">
                    <div class="table-card-title-icon" style="background:rgba(2,132,199,0.1);color:#0284C7;"><i class="fa-solid fa-user"></i></div>
                    Informasi Profil
                </div>
            </div>
            <div style="padding:1.5rem;">
                <div class="flex flex-wrap md:flex-nowrap gap-6">
                    ...
                    <div class="w-full md:w-1/3 flex flex-col items-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="w-full text-center">
                            @csrf
                            ...
                            <button type="submit" class="btn-action btn-primary mt-4 w-full" style="background:#F59E0B;color:#fff;padding:0.75rem 1rem;">
                    ...
                    <div class="w-full md:w-2/3">
                        <form action="{{ route('admin.settings.update') }}" method="POST">
                            ...
                            <button type="submit" class="btn-action btn-primary mt-4" style="padding:0.75rem 2rem;">
        ...
        <!-- Profile Photo Upload (Now Empty) -->
...
<div class="modal-overlay" id="editProductModal">
    <div class="modal-card" style="max-width:680px;">
        ...
        <div style="border-bottom:1px solid #E2E8F0;padding-bottom:1rem;margin-bottom:1.25rem;">
            <h3 style="font-weight:800;color:#0F172A;font-size:1.1rem;display:flex;align-items:center;gap:8px;">
                <i class="fa-solid fa-pen" style="color:#6366F1;"></i> Edit Produk
            </h3>
        </div>
        <form id="editProductForm" method="POST" enctype="multipart/form-data">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1.25rem;">
                <div class="form-group" style="grid-column:1/-1;">
                    <label class="form-label" for="editProductName">Nama Produk <span style="color:#EF4444;">*</span></label>
                    <input type="text" id="editProductName" name="name" required class="form-control">
                </div>
                <div class="form-group">
                    <label class="form-label" for="editProductCategory">Kategori <span style="color:#EF4444;">*</span></label>
                ...
                <div class="form-group">
                    <label class="form-label" for="editProductPrice">Harga (Rp) <span style="color:#EF4444;">*</span></label>
                    <input type="number" id="editProductPrice" name="price" required min="0" class="form-control">
                </div>
                <div class="form-group">
                    <label class="form-label" for="editProductStock">Stok <span style="color:#EF4444;">*</span></label>
                    <input type="number" id="editProductStock" name="stock" required min="0" class="form-control">
                </div>
                <div class="form-group">
                    <label class="form-label" for="editProductWeight">Berat (Gram) <span style="color:#EF4444;">*</span></label>
                    <input type="number" id="editProductWeight" name="weight" required min="0" class="form-control">
                </div>
                <div class="form-group" style="grid-column:1/-1;">
                    <label class="form-label" for="editProductDescription">Deskripsi Produk</label>
                    <textarea id="editProductDescription" name="description" rows="3" class="form-control"></textarea>
                </div>
                <div class="form-group" style="grid-column:1/-1;">
                    <label class="form-label" for="editProductImage">Gambar Produk</label>
                    <div style="display:flex;align-items:center;gap:1rem;">
                        <img id="editProductImagePreview" src="" alt="Preview" style="width:80px;height:80px;object-fit:cover;border-radius:8px;border:1px solid #E2E8F0;background:#F8FAFC;">
                        <input type="file" id="editProductImage" name="image" accept="image/*" class="form-control file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100">
                    </div>
                    <small class="text-slate-500 text-xs mt-1 block">Biarkan kosong jika tidak ingin mengubah gambar.</small>
                </div>
            </div>

            <div style="display:flex;justify-content:flex-end;gap:1rem;margin-top:1.5rem;">
                <button type="button" onclick="closeEditProductModal()" class="btn-action btn-outline">Batal</button>
...
async function openEditProductModal(productId) {
    ...
    try {
        const response = await fetch(`/admin/api/products/${productId}/edit`);
        if (!response.ok) {
        ...
        editProductWeight.value = product.weight;
        ...
        // Image preview
        if (product.image_url) { // Use the accessor
            editProductImagePreview.src = product.image_url;
        } else {
            editProductImagePreview.src = 'https://via.placeholder.com/80'; // Default placeholder
        }
        editProductImageInput.value = ''; // Clear file input

        // Set form action for update
        editProductForm.action = `/admin/product/${product.id}`;
...
function closeEditProductModal() {
    ...
    document.getElementById('editProductImagePreview').src = 'https://via.placeholder.com/80';
}
...
document.getElementById('editProductImage').addEventListener('change', function(event) {
    ...
    reader.onload = function() {
        const output = document.getElementById('editProductImagePreview');
        output.src = reader.result;
    };
    reader.readAsDataURL(event.target.files[0]);
});

// Handle edit product form submission via AJAX
document.getElementById('editProductForm').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (res.success) {
            ...
            // Reload page or update product table dynamically
            location.reload(); 
        } else {
            Swal.fire('Error!', res.message || 'Gagal memperbarui produk.', 'error');
        }
    } catch (error) {
